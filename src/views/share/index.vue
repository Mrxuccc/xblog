  ...
  <div class="share">
    ...
    <div class="share-content">
      <div class="share-content-boxList">
        <shareBox :valueList="showInfoList" :commentList="commentList" @subMitComment="subMitComment"
          @toReplys="toReplys" @toComment="toComment"></shareBox>
        <!-- 翻页器 -->
...
import type {
  setShareInfoType,
  setCommentType,
} from "@/common/ts/commonInterface";
...
const toComment = (shareid: string | number) => {
  ...
  getCommentList({ id: shareid }).then((res: any) => {
...
//回复评论
let toUserid = ref<string | number>("");
let toIds = ref<string | number>("");
const dialogFormVisible = ref<boolean>(false);
const toReplys = (toId: string | number, ids: string | number) => {
  console.log("1230");
  toUserid.value = ids;
  toIds.value = toId;
...
const submitReply = (text: string) => {
  postCommentList({
    shareid: shareids.value, //文章id
    userid: userStore().userInfo.userid, //用户id
    comment: text, //内容
    toUserid: toUserid.value, // 评论对象的用户id
